...
import { createAuthenticatedUser, createRequest, withAuthentication } from "../helpers";
...
describe("Escrow read routes", () => {
  ...
  it("lists the authenticated freelancer's persisted escrows", async () => {
    ...
    await EscrowModel.create([
      buildEscrow({ blockchainEscrowId: "1", freelancerWallet: freelancer.walletAddress }),
      buildEscrow({ blockchainEscrowId: "2", freelancerWallet: (await createAuthenticatedUser()).walletAddress })
    ]);

    const response = await withAuthentication(createRequest(app).get("/escrow"), freelancer.token)
      .expect(200);
  ...
  it("gets a persisted escrow by blockchain id and exposes existence", async () => {
    ...
    const escrow = await EscrowModel.create(
      buildEscrow({ blockchainEscrowId: "123", freelancerWallet: freelancer.walletAddress })
    );
    ...
    expect(response.body).toMatchObject({ id: escrow.id, blockchainEscrowId: "123" });
  ...
  it("returns the implemented not-found response for absent escrows", async () => {
    ...
    expect(response.body).toMatchObject({ success: false, message: "Escrow not found." });
  });
});
